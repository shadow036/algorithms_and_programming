...
int main (int argc, char *argv[]) {
  ...
  fprintf(stdout,"\nNumber of vertices visited through BFS: %d",graph_bfs_count(g,src));
  int maxDistance = -1;
  graph_bfs_distance(g,src,&maxDistance);
  fprintf(stdout,"\nThe maximum distance from the source vertex %d through BFS is %d\n",i,maxDistance);
